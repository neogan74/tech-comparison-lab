...
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/validate-kong-traefik.XXXXXX")
...
log() {
  echo "[validate-kong-traefik] $*"
}

log "Checking experiment runner syntax"
bash -n "$REPO_ROOT/experiments/api/kong-vs-traefik/run.sh"

log "Checking experiment runner help output"
"$REPO_ROOT/experiments/api/kong-vs-traefik/run.sh" --help >/dev/null

log "Checking Docker Compose config"
docker compose -f "$REPO_ROOT/deployments/docker-compose/gateway/docker-compose.yml" config --quiet
