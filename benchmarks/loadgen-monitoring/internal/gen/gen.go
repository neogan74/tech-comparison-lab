...
// RegionOf returns the region assigned to series index i, matching the
// "region" label produced by Build. It lets non-Prometheus backends name their
// series so a region-filtered query selects the same subset on both sides.
func RegionOf(i int) string { return regions[i%len(regions)] }
