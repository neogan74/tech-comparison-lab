...
type Result struct {
	...
	ThroughputOps float64 `json:"throughput_rows_sec,omitempty"`
...
func PrintTable(results []Result) {
	...
	fmt.Fprintln(w, "DB\tOperation\tRows\tp50(ms)\tp95(ms)\tp99(ms)\trows/s\tStorage")
	fmt.Fprintln(w, "--\t---------\t----\t-------\t-------\t-------\t------\t-------")
	for _, r := range results {
		storage := "-"
		if r.StorageBytes > 0 {
			storage = fmt.Sprintf("%.1fGB", float64(r.StorageBytes)/1e9)
		}
		fmt.Fprintf(w, "%s\t%s\t%d\t%.1f\t%.1f\t%.1f\t%.0f\t%s\n",
