...
// Rows are fully iterated to include network + deserialization cost (comparable to Mongo side).
func (b *Bench) Query(ctx context.Context, iterations int) ([]time.Duration, time.Duration, error) {
	...
	for i := 0; i < iterations; i++ {
		...
		// Drain rows to include deserialization cost — mirrors Mongo cursor.All behavior.
		for rows.Next() {
			var id, doc string
...
// Uses CTE since PostgreSQL has no UPDATE ... LIMIT natively.
