...
func TestComputePercentiles(t *testing.T) {
	...
	r := Compute("etcd", "write", 10000, 8, durs, 5*time.Second)
...
func TestComputeEmpty(t *testing.T) {
	r := Compute("consul", "read", 0, 1, nil, 0)
...
func TestComputeSingle(t *testing.T) {
	r := Compute("etcd", "election", 1, 1, []time.Duration{5 * time.Millisecond}, time.Second)
