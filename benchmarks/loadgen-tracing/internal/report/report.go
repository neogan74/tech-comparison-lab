...
// Result holds benchmark metrics for one backend+operation.
type Result struct {
	DB            string  `json:"db"`
	...
	ThroughputOps float64 `json:"throughput_ops_sec,omitempty"`
	TotalMs       int64   `json:"total_ms"`
}

// Summary is the top-level JSON output written per-backend.
type Summary struct {
...
func Compute(db, op string, count, iters int, durations []time.Duration, total time.Duration) Result {
	r := Result{
		DB: db, Operation: op, Count: count, Iterations: iters,
...
func PrintTable(results []Result) {
	...
	fmt.Fprintln(w, "Backend\tOperation\tCount\tp50(ms)\tp95(ms)\tp99(ms)\tOps/s")
	fmt.Fprintln(w, "-------\t---------\t-----\t-------\t-------\t-------\t-----")
