...
type Result struct {
	...
	Count        int     `json:"count"`       // total ops (commands, not pipelines)
	...
	ThroughputOps float64 `json:"throughput_ops_sec"` // commands/sec
...
// count = total commands (e.g. iterations × pipeSize for pipeline ops).
// durations = one entry per pipeline exec or per single command call.
...
// PrintTable writes a formatted side-by-side table to stdout.
func PrintTable(results []Result) {
	w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
	fmt.Fprintln(w, "DB\tOperation\tOps\tp50(ms)\tp95(ms)\tp99(ms)\tQPS\tMemory\tKeys")
	fmt.Fprintln(w, "--\t---------\t---\t-------\t-------\t-------\t---\t------\t----")
	for _, r := range results {
		...
		fmt.Fprintf(w, "%s\t%s\t%d\t%.3f\t%.3f\t%.3f\t%.0f\t%s\t%s\n",
			r.DB, r.Operation, r.Count,
			r.P50Ms, r.P95Ms, r.P99Ms,
			r.ThroughputOps, mem, keys)
	}
	w.Flush()
}
