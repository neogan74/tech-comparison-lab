#!/usr/bin/env bash
# Experiment: Redis vs Dragonfly cache benchmark
...
COMPOSE_DIR="$REPO_ROOT/deployments/docker-compose/cache-dragonfly"
...
start_stack() {
  log "Starting Docker Compose stack (cache-dragonfly)..."
  if [ "$CLEAN" = true ]; then
    log "  --clean: removing existing containers and volumes"
...
wait_for_redis() {
  log "Waiting for Redis (port 6383)..."
  local max=20 i=0
...
wait_for_dragonfly() {
  log "Waiting for Dragonfly (port 6384)..."
  local max=20 i=0
  until docker compose -f "$COMPOSE_DIR/docker-compose.yml" exec -T redis redis-cli -h dragonfly ping 2>/dev/null | grep -q PONG; do
    i=$((i+1))
    [ "$i" -ge "$max" ] && { echo "error: Dragonfly not ready" >&2; exit 1; }
    sleep 3
  done
  log "Dragonfly ready."
}
...
smoke_test() {
  ...
  "$BINARY" --db redis --op all --count "$SMOKE_COUNT" --iterations "$SMOKE_ITER" \
    --pipe-size 10 --workers 4 --flush --addr localhost:6383
  "$BINARY" --db dragonfly --op all --count "$SMOKE_COUNT" --iterations "$SMOKE_ITER" \
    --pipe-size 10 --workers 4 --flush --addr localhost:6384
...
run_full() {
  ...
  "$BINARY" \
    --db redis --op all \
    --count "$KEY_COUNT" --iterations "$ITERATIONS" \
    --pipe-size "$PIPE_SIZE" --workers "$WORKERS" \
    --flush --addr localhost:6383 --out "$RESULTS_DIR/redis.json"

  log "Running Dragonfly benchmark..."
  "$BINARY" \
    --db dragonfly --op all \
    --count "$KEY_COUNT" --iterations "$ITERATIONS" \
    --pipe-size "$PIPE_SIZE" --workers "$WORKERS" \
    --flush --addr localhost:6384 --out "$RESULTS_DIR/dragonfly.json"
}
...
merge_results() {
  ...
  jq -s '{
    ...
    experiment: {
      id: "redis-vs-dragonfly",
      name: "Redis vs Dragonfly",
      category: "cache",
      path: "experiments/cache/redis-vs-dragonfly"
    },
    run_id: .[0].run_id,
    ...
    sources: [
      ...
      {name: "dragonfly", file: "results/dragonfly.json"}
    ],
    results: [.[].results[]]
  }' "$RESULTS_DIR/redis.json" "$RESULTS_DIR/dragonfly.json" \
...
print_table() {
  ...
  echo ""
  printf "%-10s %-14s %12s %10s %10s %12s %10s %10s\n" \
  ...
  jq -r '.results[] | [
    ...
    (.p95_ms|tostring),
    (.p99_ms|tostring),
    (if .memory_used and .memory_used != "" then .memory_used else "-" end),
...
log "Prometheus: http://localhost:9097"
