...
COMPOSE_DIR="$REPO_ROOT/deployments/docker-compose/cache-memcached"
...
start_stack() {
  log "Starting Docker Compose stack (cache-memcached)..."
  if [ "$CLEAN" = true ]; then
    log "  --clean: removing existing containers"
...
wait_for_redis() {
  log "Waiting for Redis (port 6382)..."
  local max=20 i=0
...
wait_for_memcached() {
  log "Waiting for Memcached (port 11211)..."
  local max=20 i=0
  until docker compose -f "$COMPOSE_DIR/docker-compose.yml" exec -T memcached nc -z localhost 11211 2>/dev/null; do
    i=$((i+1))
    [ $i -ge $max ] && { echo "error: Memcached not ready" >&2; exit 1; }
    sleep 3
  done
  log "Memcached ready."
}
...
smoke_test() {
  ...
  "$BINARY" --db redis     --op all --count "$SMOKE_COUNT" --iterations "$SMOKE_ITER" \
    --pipe-size 10 --workers 4 --flush --addr localhost:6382
  "$BINARY" --db memcached --op all --count "$SMOKE_COUNT" --iterations "$SMOKE_ITER" \
    --pipe-size 10 --workers 4 --flush --addr localhost:11211
...
run_full() {
  ...
  log "Running Memcached benchmark..."
  "$BINARY" \
    --db         memcached \
    --op         all \
    --count      "$KEY_COUNT" \
    ...
    --out        "$RESULTS_DIR/memcached.json"
}
...
merge_results() {
  ...
  jq -s '{
    ...
    experiment: {
      ...
      category: "cache",
      path: "experiments/cache/redis-vs-memcached"
    },
    run_id: .[0].run_id,
    ...
    sources: [
      ...
      {name: "memcached", file: "results/memcached.json"}
    ],
    results: [.[].results[]]
  }' "$RESULTS_DIR/redis.json" "$RESULTS_DIR/memcached.json" \
...
print_table() {
  ...
  echo ""
  printf "%-12s %-14s %12s %10s %10s %12s %10s %10s\n" \
