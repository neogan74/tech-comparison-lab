#!/usr/bin/env bash
# Experiment: Zabbix vs Prometheus — metric ingest + query benchmark
# Usage: ./run.sh [--clean] [--smoke-only]
#
# Environment overrides:
#   COUNT           (default: 2000000   — 2M samples)
#   SERIES          (default: 2000      — unique series / trapper items)
#   INTERVAL        (default: 15        — seconds between samples of the same series)
#   BATCH_SIZE      (default: 2000      — samples per ingest request)
#   WORKERS         (default: 4)
#   QUERY_ITER      (default: 5)
#   SMOKE_COUNT     (default: 20000)
#   SMOKE_SERIES    (default: 200)
#   SMOKE_ITER      (default: 1)
#   SKIP_BUILD=true
...
COMPOSE_DIR="$REPO_ROOT/deployments/docker-compose/monitoring"
LOADGEN_DIR="$REPO_ROOT/benchmarks/loadgen-monitoring"
RESULTS_DIR="$SCRIPT_DIR/results"
BINARY="$LOADGEN_DIR/bin/loadgen-monitoring"
...
ZBX_API="http://localhost:8090/api_jsonrpc.php"
ZBX_SENDER="localhost:10051"
...
build_binary() {
  ...
  if have_cmd go; then
    log "Building loadgen-monitoring..."
...
start_stack() {
  log "Starting Docker Compose stack (monitoring)..."
  if [ "$1" = "clean" ]; then
...
wait_for_zabbix() {
  log "Waiting for Zabbix API (port 8090)..."
  local max=60 i=0
  local body='{"jsonrpc":"2.0","method":"apiinfo.version","params":{},"id":1}'
  until curl -sf -X POST -H 'Content-Type: application/json-rpc' \
      -d "$body" "$ZBX_API" 2>/dev/null | grep -q '"result"'; do
    i=$((i+1))
    [ $i -ge $max ] && { echo "error: Zabbix API not ready after 300s" >&2; exit 1; }
    sleep 5
  done
  log "Zabbix API ready."
}
...
run_prometheus() {
  local out="$1"; shift
  "$BINARY" --db prometheus --op all "$@" --addr "$PROM_ADDR" ${out:+--out "$out"}
}

run_zabbix() {
  local out="$1"; shift
  "$BINARY" --db zabbix --op all "$@" \
    --addr "$ZBX_API" --zbx-sender "$ZBX_SENDER" ${out:+--out "$out"}
}

smoke_test() {
  log "--- Smoke test (${SMOKE_COUNT} samples, ${SMOKE_SERIES} series, ${SMOKE_ITER} query iters) ---"
  run_prometheus "" --count "$SMOKE_COUNT" --series "$SMOKE_SERIES" \
    --interval "$INTERVAL" --batch 1000 --workers 2 --query-iter "$SMOKE_ITER"
  run_zabbix "" --count "$SMOKE_COUNT" --series "$SMOKE_SERIES" \
    --interval "$INTERVAL" --batch 1000 --workers 2 --query-iter "$SMOKE_ITER"
...
run_full() {
  ...
  run_prometheus "$RESULTS_DIR/prometheus.json" \
    --count "$COUNT" --series "$SERIES" --interval "$INTERVAL" \
    --batch "$BATCH_SIZE" --workers "$WORKERS" --query-iter "$QUERY_ITER"

  log "Running Zabbix benchmark..."
  run_zabbix "$RESULTS_DIR/zabbix.json" \
    --count "$COUNT" --series "$SERIES" --interval "$INTERVAL" \
    --batch "$BATCH_SIZE" --workers "$WORKERS" --query-iter "$QUERY_ITER"
}
...
merge_results() {
  ...
  jq -s '{
    ...
    experiment: {
      id: "zabbix-vs-prometheus",
      name: "Zabbix vs Prometheus",
      category: "monitoring",
      path: "experiments/monitoring/zabbix-vs-prometheus"
    },
    run_id: .[0].run_id,
    ...
    sources: [
      ...
      {name: "zabbix", file: "results/zabbix.json"}
    ],
    results: [.[].results[]]
  }' "$RESULTS_DIR/prometheus.json" "$RESULTS_DIR/zabbix.json" \
...
print_table() {
  ...
  echo ""
  printf "%-12s %-12s %10s %10s %10s %12s %10s\n" \
...
# Restart with fresh volumes so the full run's data isn't mixed with the
# smoke-test data (and Zabbix items are recreated cleanly).
...
log "Zabbix API:  $ZBX_API"
log "Zabbix web:  http://localhost:8090 (Admin / zabbix)"
log "Grafana:     http://localhost:3007"
