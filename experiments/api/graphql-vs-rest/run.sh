...
GRAPHQL_ADDR="http://localhost:8080"
...
info "Starting bench-api (REST+GraphQL :8080)..."
"$API_BIN" &
SERVER_PID=$!

# wait until REST responds (GraphQL shares the same server)
...
"$BENCH_BIN" --proto graphql --op all --count "$SMOKE_COUNT" --workers 10 --addr "$GRAPHQL_ADDR" \
  --out "$RESULTS_DIR/smoke-graphql.json"

[[ -s "$RESULTS_DIR/smoke-rest.json" ]]    || { echo "ERROR: smoke-rest.json missing" >&2; exit 1; }
[[ -s "$RESULTS_DIR/smoke-graphql.json" ]] || { echo "ERROR: smoke-graphql.json missing" >&2; exit 1; }
...
info "GraphQL benchmark..."
"$BENCH_BIN" --proto graphql --op all \
  --count "$COUNT" --workers "$WORKERS" \
  --addr "$GRAPHQL_ADDR" \
  --out "$RESULTS_DIR/graphql.json"

# ── side-by-side summary ──────────────────────────────────────────────────────
if [[ -f "$RESULTS_DIR/rest.json" && -f "$RESULTS_DIR/graphql.json" ]]; then
  echo ""
  echo "════════════════════════════════════════════════════════════════════════"
  echo " GraphQL vs REST — side-by-side (RPS, p50/p99 ms)"
  echo "════════════════════════════════════════════════════════════════════════"
  printf "%-20s  %8s %8s %8s %8s %8s %8s\n" "operation" "REST RPS" "GQL RPS" "REST p50" "GQL p50" "REST p99" "GQL p99"
  printf "%-20s  %8s %8s %8s %8s %8s %8s\n" "─────────────────────" "────────" "────────" "────────" "────────" "────────" "────────"
  ...
  for op in $ops; do
    ...
    g_rps=$(jq -r --arg op "$op" '.results[] | select(.op==$op) | .throughput_rps' "$RESULTS_DIR/graphql.json" | xargs printf "%.0f")
    r_p50=$(jq -r --arg op "$op" '.results[] | select(.op==$op) | .p50_ms' "$RESULTS_DIR/rest.json" | xargs printf "%.2f")
    g_p50=$(jq -r --arg op "$op" '.results[] | select(.op==$op) | .p50_ms' "$RESULTS_DIR/graphql.json" | xargs printf "%.2f")
    r_p99=$(jq -r --arg op "$op" '.results[] | select(.op==$op) | .p99_ms' "$RESULTS_DIR/rest.json" | xargs printf "%.2f")
    g_p99=$(jq -r --arg op "$op" '.results[] | select(.op==$op) | .p99_ms' "$RESULTS_DIR/graphql.json" | xargs printf "%.2f")
...
jq -s '{
  ...
  experiment: {
    ...
    category: "api",
    path: "experiments/api/graphql-vs-rest"
  },
  run_id: .[0].run_id,
  ...
  sources: [
    ...
    {name: "graphql", file: "results/graphql.json"}
  ],
  results: [.[].results[]]
}' \
  "$RESULTS_DIR/rest.json" "$RESULTS_DIR/graphql.json" \
