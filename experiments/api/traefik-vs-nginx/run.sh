...
COMPOSE_DIR="$REPO_ROOT/deployments/docker-compose/proxy"
RESULTS_DIR="$SCRIPT_DIR/results"

API_BIN="$API_DIR/bench-api"
BENCH_BIN="$BENCH_DIR/loadgen-http"

NGINX_ADDR="http://localhost:8090"
TRAEFIK_ADDR="http://localhost:8091"
...
cleanup_stack() {
  if [[ "$KEEP_STACK" == "1" ]]; then
    warn "KEEP_STACK=1 — Traefik + NGINX stack left running."
    return
  fi
  info "Stopping Traefik + NGINX stack..."
  docker compose -f "$COMPOSE_DIR/docker-compose.yml" down --remove-orphans >/dev/null 2>&1 || true
}

start_stack() {
  if [[ "$CLEAN" == true ]]; then
    info "Removing existing proxy stack..."
    docker compose -f "$COMPOSE_DIR/docker-compose.yml" down --remove-orphans >/dev/null 2>&1 || true
  fi
  info "Starting Traefik + NGINX stack..."
...
wait_proxy "Traefik" "$TRAEFIK_ADDR"
...
if [[ -f "$RESULTS_DIR/nginx.json" && -f "$RESULTS_DIR/traefik.json" ]]; then
  echo ""
  echo "════════════════════════════════════════════════════════════════════════"
  echo " Traefik vs NGINX — side-by-side (RPS, p50/p99 ms)"
  echo "════════════════════════════════════════════════════════════════════════"
  printf "%-20s  %8s %8s %8s %8s %8s %8s\n" "operation" "NGX RPS" "TRF RPS" "NGX p50" "TRF p50" "NGX p99" "TRF p99"
  printf "%-20s  %8s %8s %8s %8s %8s %8s\n" "─────────────────────" "────────" "────────" "────────" "────────" "────────" "────────"
  ...
  for op in $ops; do
    ...
    printf "%-20s  %8s %8s %8s %8s %8s %8s\n" "$op" "$n_rps" "$t_rps" "$n_p50" "$t_p50" "$n_p99" "$t_p99"
  done
  echo "════════════════════════════════════════════════════════════════════════"
fi
...
jq -s '{
  ...
  experiment: {
    id: "traefik-vs-nginx",
    name: "Traefik vs NGINX",
    category: "api",
    path: "experiments/api/traefik-vs-nginx"
  },
  run_id: .[0].run_id,
  ...
  "$RESULTS_DIR/nginx.json" "$RESULTS_DIR/traefik.json" \
  > "$RESULTS_DIR/summary.json"
[[ -s "$RESULTS_DIR/summary.json" ]] || { echo "ERROR: summary.json missing" >&2; exit 1; }
info "Summary saved to $RESULTS_DIR/summary.json"

info "NGINX proxy:   http://localhost:8090"
info "Traefik proxy: http://localhost:8091  (dashboard: http://localhost:8092/dashboard/)"
info "Done."
