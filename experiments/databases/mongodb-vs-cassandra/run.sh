#!/usr/bin/env bash
# Experiment: MongoDB vs Cassandra distributed NoSQL workload benchmark
...
COMPOSE_DIR="$REPO_ROOT/deployments/docker-compose/mongodb-vs-cassandra"
...
Environment overrides:
  ...
  WORKERS, BATCH_SIZE, SMOKE_COUNT, MONGO_PASSWORD
  SMOKE_ONLY=true
...
load_env() {
  ...
  MONGO_DSN="mongodb://bench:${MONGO_PASSWORD}@localhost:${MONGO_PORT}/?authSource=admin"
...
start_stack() {
  ...
  log "Starting MongoDB and Cassandra..."
  docker compose -f "$COMPOSE_DIR/docker-compose.yml" up -d mongo cassandra
}
...
wait_for_mongo() {
  ...
  until docker compose -f "$COMPOSE_DIR/docker-compose.yml" exec -T mongo \
      mongosh --quiet --username bench --password "$MONGO_PASSWORD" \
      --authenticationDatabase admin --eval "db.adminCommand('ping').ok" >/dev/null 2>&1; do
    i=$((i+1))
    [ "$i" -lt 36 ] || { echo "error: MongoDB did not become ready in 180s" >&2; exit 1; }
    sleep 5
  done
}
...
run_target() {
  ...
  local dsn=$MONGO_DSN
  [ "$db" = cassandra ] && dsn=$CASSANDRA_DSN
...
run_full() {
  ...
  run_target mongo "$INSERT_COUNT" "$RESULTS_DIR/mongo.json"
...
merge_results() {
  jq -s '{
    ...
    experiment: {
      id: "mongodb-vs-cassandra",
      name: "MongoDB vs Cassandra",
      category: "databases",
      path: "experiments/databases/mongodb-vs-cassandra"
    },
    run_id: .[0].run_id,
    ...
    results: [.[].results[]]
  }' "$RESULTS_DIR/mongo.json" "$RESULTS_DIR/cassandra.json" > "$RESULTS_DIR/summary.json"
}

verify_results() {
  for path in "$RESULTS_DIR/mongo.json" "$RESULTS_DIR/cassandra.json" "$RESULTS_DIR/summary.json"; do
