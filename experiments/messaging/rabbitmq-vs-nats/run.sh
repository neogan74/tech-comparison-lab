...
COMPOSE_DIR="$REPO_ROOT/deployments/docker-compose/rabbitmq-vs-nats"
...
start_stack() {
  log "Starting Docker Compose stack (rabbitmq-vs-nats)..."
  if [ "$CLEAN" = true ]; then
    docker compose -f "$COMPOSE_DIR/docker-compose.yml" down -v --remove-orphans 2>/dev/null || true
  fi
  docker compose -f "$COMPOSE_DIR/docker-compose.yml" up -d rabbitmq nats
}

wait_for_rabbitmq() {
  log "Waiting for RabbitMQ (port 5672)..."
  local max=60 i=0
  until "$BINARY" --db rabbitmq --dry-run --addr "$RABBIT_ADDR" &>/dev/null; do
    i=$((i+1))
...
wait_for_nats() {
  log "Waiting for NATS (port 4222)..."
  local max=24 i=0
  until "$BINARY" --db nats --dry-run --addr "$NATS_ADDR" &>/dev/null; do
    i=$((i+1))
    [ $i -ge $max ] && { echo "error: NATS not ready after 120s" >&2; exit 1; }
    sleep 5
  done
  log "NATS ready."
}
...
smoke_test() {
  ...
  "$BINARY" --db rabbitmq --op all --count "$SMOKE_COUNT" --batch 100 \
    ...
    --addr "$RABBIT_ADDR"
  "$BINARY" --db nats --op all --count "$SMOKE_COUNT" --batch 100 \
    --consumers "$CONSUMERS" --clean \
    --addr "$NATS_ADDR"
...
merge_results() {
  ...
  jq -s '{
    ...
    experiment: {
      id: "rabbitmq-vs-nats",
      name: "RabbitMQ vs NATS",
      category: "messaging",
      path: "experiments/messaging/rabbitmq-vs-nats"
    },
    run_id: .[0].run_id,
    ...
    sources: [
      ...
      {name: "nats",     file: "results/nats.json"}
    ],
    results: [.[].results[]]
  }' "$RESULTS_DIR/rabbitmq.json" "$RESULTS_DIR/nats.json" \
...
log "RabbitMQ Management: http://localhost:15672 (bench/benchpass)"
