#!/usr/bin/env bash
# Apache Kafka vs Apache Pulsar high-throughput messaging benchmark
...
COMPOSE_DIR="$REPO_ROOT/deployments/docker-compose/kafka-vs-pulsar"
...
KAFKA_ADDR="${KAFKA_ADDR:-localhost:9093}"
PULSAR_ADDR="${PULSAR_ADDR:-pulsar://localhost:6650}"
PULSAR_TOPIC="persistent://public/default/bench"
...
start_stack() {
  log "Starting Docker Compose stack (kafka-vs-pulsar)..."
  if [ "$CLEAN" = true ]; then
    docker compose -f "$COMPOSE_DIR/docker-compose.yml" down -v --remove-orphans 2>/dev/null || true
  fi
  docker compose -f "$COMPOSE_DIR/docker-compose.yml" up -d kafka pulsar
}
...
wait_for_pulsar() {
  log "Waiting for Pulsar (port 6650)..."
  local max=30 i=0
  until docker compose -f "$COMPOSE_DIR/docker-compose.yml" exec -T pulsar \
      bin/pulsar-admin brokers healthcheck &>/dev/null; do
    i=$((i+1))
    [ "$i" -ge "$max" ] && { echo "error: Pulsar not ready after 300s" >&2; exit 1; }
    sleep 10
  done
  log "Pulsar ready."
}

prepare_pulsar_topic() {
  log "Recreating Pulsar topic with $PARTITIONS partitions..."
  docker compose -f "$COMPOSE_DIR/docker-compose.yml" exec -T pulsar \
    bin/pulsar-admin topics delete-partitioned-topic "$PULSAR_TOPIC" --force &>/dev/null || true
  docker compose -f "$COMPOSE_DIR/docker-compose.yml" exec -T pulsar \
    bin/pulsar-admin topics create-partitioned-topic "$PULSAR_TOPIC" --partitions "$PARTITIONS"
}
...
smoke_test() {
  ...
  prepare_pulsar_topic
  "$BINARY" --db pulsar --op all --count "$SMOKE_COUNT" --batch 100 \
    --consumers "$CONSUMERS" --partitions "$PARTITIONS" \
    --topic "$PULSAR_TOPIC" --addr "$PULSAR_ADDR"
...
run_full() {
  ...
  "$BINARY" \
    --db kafka --op all --count "$MSG_COUNT" --batch "$BATCH_SIZE" \
    --consumers "$CONSUMERS" --partitions "$PARTITIONS" --clean \
    --addr "$KAFKA_ADDR" --out "$RESULTS_DIR/kafka.json"

  prepare_pulsar_topic
  log "Running Pulsar benchmark..."
  "$BINARY" \
    --db pulsar --op all --count "$MSG_COUNT" --batch "$BATCH_SIZE" \
    --consumers "$CONSUMERS" --partitions "$PARTITIONS" \
    --topic "$PULSAR_TOPIC" --addr "$PULSAR_ADDR" \
    --out "$RESULTS_DIR/pulsar.json"
}
...
merge_results() {
  ...
  jq -s '{
    ...
    experiment: {
      id: "kafka-vs-pulsar",
      name: "Apache Kafka vs Apache Pulsar",
      category: "messaging",
      path: "experiments/messaging/kafka-vs-pulsar"
    },
    run_id: .[0].run_id,
    ...
    sources: [
      ...
      {name: "pulsar", file: "results/pulsar.json"}
    ],
    results: [.[].results[]]
  }' "$RESULTS_DIR/kafka.json" "$RESULTS_DIR/pulsar.json" > "$RESULTS_DIR/summary.json"
}

verify_results() {
  for path in "$RESULTS_DIR/kafka.json" "$RESULTS_DIR/pulsar.json" "$RESULTS_DIR/summary.json"; do
    if [ ! -s "$path" ]; then
...
log "Prometheus:   http://localhost:9096"
log "Grafana:      http://localhost:3004"
