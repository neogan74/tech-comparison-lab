#!/usr/bin/env bash
# Experiment #9: NATS vs Kafka high-throughput messaging benchmark
...
COMPOSE_DIR="$REPO_ROOT/deployments/docker-compose/nats-vs-kafka"
...
start_stack() {
  log "Starting Docker Compose stack (nats-vs-kafka)..."
  if [ "$CLEAN" = true ]; then
    docker compose -f "$COMPOSE_DIR/docker-compose.yml" down -v --remove-orphans 2>/dev/null || true
  fi
  docker compose -f "$COMPOSE_DIR/docker-compose.yml" up -d kafka nats
}
...
merge_results() {
  ...
  jq -s '{
    ...
    experiment: {
      id: "nats-vs-kafka",
      name: "NATS vs Apache Kafka",
      category: "messaging",
      path: "experiments/messaging/nats-vs-kafka"
    },
    run_id: .[0].run_id,
    ...
    results: [.[].results[]]
  }' "$RESULTS_DIR/kafka.json" "$RESULTS_DIR/nats.json" \
